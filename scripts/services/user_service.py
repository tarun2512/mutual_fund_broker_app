...
from scripts.errors import CustomError, LicenceValidationError, FixedDelayError, VariableDelayError, \
...
UserRegister, LoginRequest, ResetPasswordRequest,
)

user_router = APIRouter(prefix=APIEndpoints.proxy_user, tags=["User Service"])

@user_router.post(APIEndpoints.api_create_user, response_model=DefaultResponse, name="create_user")
def create_user(
    ...
    try:
        ...
        response = user_handler.register_user(request_data)
        ...
        return DefaultFailureResponse(message="Failed while creating user", error=str(e)).model_dump()


@user_router.post(APIEndpoints.api_reset_password, include_in_schema=False, name="reset_password")
def reset_password(request_data: ResetPasswordRequest, request: Request):
    """
    ...
    try:
        user_handler = UserHandler()
        resp = user_handler.reset_password(request_data.model_dump())
...
def login(
    ...
    try:
        ...
        user_handler = UserHandler()
        resp = user_handler.login_flow(
            login_request=request_data,
            request=request,
            response=response,
            bg_task=background_tasks,
        )
        return {"status": "success", "message": "Successfully Logged in", "data": resp} or {"status": "failed", "message": DefaultExceptionsCode.DE001}
...
def logout(request: Request):
    ...
    try:
        ...
        user_handler = UserHandler()
        return user_handler.logout(session_id, login_token, refresh_token)
    except Exception as e:
