...
from scripts.db.mongo.mutual_fund_store.user_portfolio import (
    UserPortfolio,
    UserPortfolioSchema,
)
...
class MutualFundBrokerHandler:
    ...
    async def fetch_nav_value(self, scheme_code):
        try:
    ...
    async def add_funds_to_portfolio(self, request_data: AddFunds):
        try:
            fetch_user_portfolio = await self.user_portfolio_con.find_user_portfolio(
                user_id=request_data.user_id, scheme_code=request_data.scheme_code
            )
            request_data.nav = await self.fetch_nav_value(request_data.scheme_code)
            if not fetch_user_portfolio:
                ...
                await self.user_portfolio_con.update_user_portfolio(
                    {}, data=portfolio_record.model_dump()
                )
            else:
                new_units = request_data.amount / request_data.nav
                updated_units_held = (
                    fetch_user_portfolio.get("units_held", 0) + new_units
                )
                new_total = (
                    fetch_user_portfolio.get("units_held")
                    * fetch_user_portfolio.get("average_nav_price")
                    + new_units * request_data.nav
                )
                new_avg_nav = new_total / updated_units_held
                await self.user_portfolio_con.update_user_portfolio(
                    query={"user_id": request_data.user_id},
                    data={
                        "units_held": updated_units_held,
                        "average_nav_price": new_avg_nav,
                        "last_investment_on": int(time.time() * 1000),
                    },
                )
        except Exception as e:
    ...
    async def fetch_user_portfolio(self, user_id):
        try:
            records = await self.user_portfolio_con.find_user_portfolio(user_id=user_id)
            for record in records:
                record["current_nav_price"] = await self.fetch_nav_value(
    ...
    async def fetch_mutual_fund_family_data(self):
        try:
            records = await self.mutual_fund_data_con.fetch_records()
            mutual_fund_data = []
            for record in records:
    ...
    async def fetch_hourly_portfolio_data(self, user_id: str) -> dict[str, dict]:
        """
        ...
        try:
            # ---- 1. DB reads ----------------------------------------------------
            user_portfolio = await self.user_portfolio_con.find_user_portfolio(
                user_id=user_id
            )
            hourly_snapshots = (
                await self.user_portfolio_hourly_con.find_user_portfolio_hourly(
                    user_id=user_id
                )
            )
