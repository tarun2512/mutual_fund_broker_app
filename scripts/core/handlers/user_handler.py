...
class UserHandler:
    ...
    async def register_user(self, request_data: UserRegister):
        try:
            request_data.email = request_data.email.lower()
            existing_user_name = await self.user_con.find_user(
                username=request_data.username
            )

            if request_data.user_id:
                return await self.edit_user(
                    request_data,
                    existing_user_name,
                )
            else:
                return await self.add_user(existing_user_name, request_data)
        except Exception as e:
            logger.exception(str(e))
            raise

    async def edit_user(self, input_data: UserRegister, existing_user_name):
        ...
        if "password" not in input_data or not input_data.password:
            existing_user = await self.user_con.find_user(user_id=user_id)
            existing_user = (
                existing_user.dict() if bool(existing_user) else existing_user
            )
            input_data.password = existing_user["password"]
        await self.user_con.update_one_user(
            {"user_id": input_data.user_id}, data=input_data.model_dump()
        )
        return input_data.user_id

    async def add_user(self, existing_user_name, input_data):
        ...
        await self.user_con.insert_one_user(input_data.model_dump())
    ...
    async def login_flow(
        ...
        try:
            # Get user details
            user_record = await self.get_user_record(username=login_request.username)
            # Check for failed login attempts
            if user_record.get("is_user_locked", False):
                raise CustomError(UserExceptions.USER_INACTIVE)
            auth = await self.user_handler_helper.validate_password(
                password=login_request.password,
                user_record=user_record,
            )
            if not auth:
                await self.user_handler_helper.reset_login_attempts(
                    user_record, reset=False
                )
                raise InvalidPasswordError(msg=DefaultExceptionsCode.DEIP)
            client_ip = request.client.host
            # Login Response
            await self.common_method_login_token(
                response=response,
                user_record=user_record,
                client_ip=client_ip,
            )
    ...
    async def get_user_record(self, username):
        try:
            user_record = await self.user_con.find_user(
                username=username, filter_dict={"_id": 0}
            )
    ...
    async def common_method_login_token(self, response, user_record, client_ip):
        ...
        await self.user_handler_helper.set_login_token(
        ...
        await self.user_handler_helper.set_user_id(
            response=response, user_id=user_record.get("user_id")
        )

    async def reset_password(self, request_data):
        try:
            ...
            for each_record in user_record:
                ...
                if validate_password_strength(request_data["new_password"]):
                    ...
                    new_values = {
                        ...
                        "password_added_on": int(time.time()),
                    }
                    await self.user_con.update_one_user(
                    ...
                    await self.user_handler_helper.reset_login_attempts(each_record)
                    ...
                    logger.error(
                        f"Error occurred while occurred while resetting password : {error_msg}"
                    )
                    resp_json = {
                        "status": "failed",
                        "message": error_msg,
    ...
    async def logout(self, session_id, login_token, refresh_token):
        ...
        try:
            ...
            resp = Response(
                content=json.dumps(final_json), media_type="application/json"
            )
            resp.set_cookie(
                "user_id",
                "",
